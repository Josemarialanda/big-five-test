...
import Utils.Utils (matchToRegex, parseNumber, printTag, removeRepeatingSeparator)
...
parseResults =
  results
    . fmap existentialFacet -- Convert list of existentially typed facets to a Results
    . S.chunksOf 7 -- Convert list of facet text into a list of existentially typed facets
    . (fmap . removeRepeatingSeparator $ '.') -- 7 because each facet has 6 scores and 1 overall score
    -- Remove repeating separators from testResultsRegex
  where
    ...
    facetScores xs = case compare (length xs) 6 of
      ...
      EQ ->
        let [(_, s1), (_, s2), (_, s3), (_, s4), (_, s5), (_, s6)] = xs
